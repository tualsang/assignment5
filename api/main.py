...
from sqlalchemy.testing.engines import reconnecting_engine
...
def create_order_details(order_d: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return order_details.create(db=db, order_details=order_d)
...
def update_one_order_detail(order_detail_id: int, order_detail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    ...
    if order_detail_db is None:
        raise HTTPException(status_code=404, detail="Order Detail not found")
    return order_details.update(db=db, order_detail_id=order_detail_id, order=order_detail)
